...
# GROUP MEMBERS:
# LAST NAME,        FIRST NAME,     STUDENT-ID,     GITHUB-ID
# Perminova,        Marina,         20-620-928,     MarinaPerminova, 
# Bader,            Martin,         12-611-570,     tooempty, 
# Moine,            Alexandre,      15-052-319,     GoldenPineappleR
---
...
# since the name of Refinitive columns is too long, we need to remove the (FY0) part -> changed it directly on the file //TO DO// see if it works
```

```{r Comparison ESG Refinitive Score and ESG wording, echo = FALSE}
...
The average Flesch readability score for the energy companies is at around `r round(mean(energy_rd_flesch$Flesch), digits = 2)`, indicating very complex and difficult to read text sources.

```{r Plot readability Flesch for energy, echo=FALSE}
# all companies, all years
ggplotly(ggplot(energy_rd_flesch, aes(x=energy_10k$year, y =Flesch, colour=energy_10k$cik)) + geom_point() + ggtitle("Flesch - Readability for energy companies"), tooltip = c("colour", "y", "x"))
```

The average FOG score for our sample of 30 energy companies from the US is `r round(mean(energy_rd_fog$FOG), digits = 2)`.

```{r Readability plots FOG for energy, echo = FALSE, fig.width = 10 , fig.fullwidth=TRUE}
# all companies, all years
ggplotly(ggplot(energy_rd_fog, aes(x=energy_10k$year, y =FOG, color=energy_10k$cik)) + geom_point() + ggtitle("FOG - Readability for fashion companies"), tooltip = c("colour", "y", "x"))
```
...
# all companies, all years
ggplotly(ggplot(bev_rd_flesch, aes(x=bev_10k$year, y =Flesch, colour=bev_10k$cik)) + geom_point() + ggtitle("Flesch - Readability for beverage companies"), tooltip = c("colour", "y", "x"))
```
...
# all companies, all years
ggplotly(ggplot(bev_rd_fog, aes(x=bev_10k$year, y =FOG, color=bev_10k$cik)) + geom_point() + ggtitle("FOG - Readability for beverage companies"), tooltip = c("colour", "y", "x"))
```
...
bev_csr_env <- plot_ly(data = bev_CSR, x = ~year, y = ~relative_env, color = ~cik) %>% 
        add_markers() %>%
        layout(title = 'Share of environment wording. CSR dictionary. Beverages',
              xaxis = list(title = 'year'),
              yaxis = list(title = 'percentage', hoverformat = '.2f'))
bev_csr_env

bev_csr_soc <- plot_ly(data = bev_CSR, x = ~year, y = ~relative_soc, color = ~cik) %>% 
        add_markers() %>%
        layout(title = 'Share of social wording. CSR dictionary. Beverages',
              xaxis = list(title = 'year'),
              yaxis = list(title = 'percentage', hoverformat = '.2f'))
bev_csr_soc

bev_csr_empl <- plot_ly(data = bev_CSR, x = ~year, y = ~relative_empl, color = ~cik) %>% 
        add_markers() %>%
        layout(title = 'Share of employee-related wording. CSR dictionary. Beverages',
              xaxis = list(title = 'year'),
              yaxis = list(title = 'percentage', hoverformat = '.2f'))
bev_csr_empl

bev_csr_humans <- plot_ly(data = bev_CSR, x = ~year, y = ~relative_humans, color = ~cik) %>% 
       add_markers() %>%
        layout(title = 'Share of human-related wording. CSR dictionary. Beverages',
              xaxis = list(title = 'year'),
              yaxis = list(title = 'percentage', hoverformat = '.2f'))
bev_csr_humans

bev_csr_total <- plot_ly(data = bev_CSR, x = ~year, y = ~relative_overall, color = ~cik) %>% 
      add_markers() %>%      
      layout(title = 'Share of ESG wording. CSR dictionary. Beverages',
              xaxis = list(title = 'year'),
              yaxis = list(title = 'percentage', hoverformat = '.2f'))
bev_csr_total
```
...
bev_bbk_env <- plot_ly(data = bev_BBK, x = ~year, y = ~relative_env, color = ~cik) %>% 
  add_markers() %>%
        layout(title = 'Share of environment wording. BBK dictionary. Beverages',
              xaxis = list(title = 'year'),
              yaxis = list(title = 'percentage', hoverformat = '.2f'))
bev_bbk_env

bev_bbk_soc <- plot_ly(data = bev_BBK, x = ~year, y = ~relative_soc, color = ~cik) %>% 
  add_markers() %>%
        layout(title = 'Share of social wording. BBK dictionary',
              xaxis = list(title = 'year'),
              yaxis = list(title = 'percentage', hoverformat = '.2f'))
...
bev_bbk_govern <- plot_ly(data = bev_BBK, x = ~year, y = ~relative_govern, color = ~cik) %>% 
  add_markers() %>%
        layout(title = 'Share of corporate governance wording. BBK dictionary',
              xaxis = list(title = 'year'),
              yaxis = list(title = 'percentage', hoverformat = '.2f'))
...
bev_bbk_total <- plot_ly(data = bev_BBK, x = ~year, y = ~relative_overall, color = ~cik) %>% 
  add_markers() %>%
        layout(title = 'Share of ESG wording. CSR dictionary',
              xaxis = list(title = 'year'),
              yaxis = list(title = 'percentage', hoverformat = '.2f'))
...
```{r Time evolution for fashion, echo=FALSE}
