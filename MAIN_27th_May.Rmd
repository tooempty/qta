...
But regardless of how well a company actually adheres to ESG-Standards, it will usually present its actions in a favorable way.
...
# all companies, all years
ggplotly(ggplot(fashion_rd_fog, aes(x=fashion_10k$year, y =FOG, color=fashion_10k$cik)) + coord_cartesian(ylim = c(15, 38)) + geom_point() + ggtitle("FOG - Readability for fashion companies"), tooltip = c("colour", "y", "x"))
```
...
ESG_scores_fashion_rf_panel_plot <- ESG_scores_fashion_rf_panel %>% group_by(cik) %>%
                                          summarise(mean_rating = mean(rating),
                                          mean_total = mean(relative_overall))

plot_ly(ESG_scores_fashion_rf_panel_plot) %>% 
      add_markers(x = ~cik,  y = ~mean_rating) %>%  
      add_markers(x = ~cik, y = ~mean_total) %>%    
      layout(title = 'ESG rating vs wroding. Fashion',
              xaxis = list(title = 'company'),
              yaxis = list(title = 'rating', tickformat = '.0', hoverformat = '.1f', showline = FALSE, 
                           side = "left", color = "blue", fixedrange=TRUE))
```
...
# all companies, all years
ggplotly(ggplot(energy_rd_fog, aes(x=energy_10k$year, y =FOG, color=energy_10k$cik)) + geom_point() + coord_cartesian(ylim = c(15, 38)) + ggtitle("FOG - Readability for energy companies"), tooltip = c("colour", "y", "x"))
```
...
# all companies, all years
ggplotly(ggplot(bev_rd_fog, aes(x=bev_10k$year, y =FOG, color=bev_10k$cik)) + geom_point() + coord_cartesian(ylim = c(15, 38))+ ggtitle("FOG - Readability for beverage companies"), tooltip = c("colour", "y", "x"))
```
